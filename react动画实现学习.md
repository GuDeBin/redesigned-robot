...
第三个 Demo 出现了拖拽动画失效，应该是放置时，也就是 drop 时动画没有移动

原因到底是出在拖拽库还是动画没有执行上，之前的两个 Demo 都是可以运行的，但是第二个 demo 其实是 flipped 自己新增的而不是默认的 ref

还是没找到问题，但是我已经有些疲惫了，这个问题根源在于我压根不理解 React 的原理和代码量太少，经验值只是一个新手村的玩家，刷小怪直接遇到 boss，这个，死定了

但是我还是留下一个我认为最有可能的解释，那就是这个动画没有执行，而不是拖拽库。
