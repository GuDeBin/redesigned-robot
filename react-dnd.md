...
现在遇到遇到的一个问题，我按照 React-DnD 教程去做一个 Demo，按照官网提供的代码是可以正常运行，但是我做了一个类似，却只能运行一次

简单说下这个是什么，React-Dnd 是一个拖放库，它提供简易的方式实现拖拽特效

先说下我和它的区别

教程

1. 他将棋子逻辑，也就是走日这个逻辑放在一个单独的文件 Game 中，用一个类，类包含一个棋子的位置，还有一个 observer，英文意思是观察员，我观察整个逻辑，发现这个 observer 函数大致有这个功能，传入一个参数，这个参数就是用 useState 的更新 state 函数，下面他首先是添加到类中的一个 observers 数组中，执行一个 emitChange 函数，这个 emitChange 的意思是发出改变

在这个 emitChange 函数中，有两个语句被执行，一个是获取最新的位置信息，还有一个遍历 observers 数组里的更新 state 函数，也就是设置棋子位置的函数，不过在遍历元素时，先确认函数存在后，将最新的位置数组传入到更新函数中执行。这里又有疑问，为什么要遍历呢，难道有不同的更新 state 函数，还存在 undefined 函数或者 null。

还有就是 observer 最后时返回一个函数，一个将原来的 observers 数组过滤，将不同于传入的更新 state 函数过滤出来，将传入的过滤掉，这有什么用呢，而且只是更新 setKnightPosition 的函数数组，还把之前的传入的去除掉，这个意思是只留最初的那个吗，还有按照语句逻辑，限制性的是移动位置，也就是 emitChange，最后将删除掉这个刚刚传入的元素，既然如此，为何要用数组，基于什么考虑，性能吗

这是一个反复重置的意思吗

关键在于 useEffect 的执行方式

那如果我加上了这个呢，按照我之前的想法，它在 App 上新建了一个类的示例，还有就是如何可以看到这个类是不是就能知道他到底是什么

我先打印一次这个 game

实在是太卡了

先描述下我和他们的不同，也就是我在，首先是顶端 App 的类的实例，一个 effect 函数，还有就是在棋盘上的

这次轮到我懵逼了，他保存了所有的驱动修改状态的函数，而且还一一调用了，这到底什么操作

副作用，useMemo

设计思路，既然这个逻辑对内部状态的重新渲染，也就是说如果我将所有的渲染逻辑收集起来。。。。再将这个方法放在 effect 中，也就是将 observe 这个函数放在 effect 内呢，可以试一试

暂时不再纠结这个，尝试后就开始之前的拖拽尝试

留下两个问题

与其说是问题，倒不如说是两个空白，一个是 react-dnd 的拖拽原理，也就是 api 各个参数的意义，还有就是 react 中 effect 和 memo 有什么作用

ahook 也有一个拖拽库，是不是可以用他的 api 实现一次呢

突然想到一个思路，到底是什么驱动了拖拽库的特效，是 game 这个函数吗
...
暂时告一段落，一开始目的本来是想了解下 react-dnd，但是一个教程牵扯出很多的知识点，至少对于小白的我还是很多，知识很多，只取自己需要的才能完成一件事。

useMemo、useEffect

将业务逻辑、UI 渲染逻辑剥离，如何用业务逻辑的数据驱动 UI 的渲染，这个还得在后面的项目继续思考

一个项目一开始如何，后续如何升级，一个最初的 idea 如何长成参天大树。

具体的原型图、构思项目、分解任务、阶段完成，测试，上线
